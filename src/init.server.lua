...
if game:GetService("CoreGui"):FindFirstChild("SetupGUI-WSRT") ~= nil then
	game:GetService("CoreGui"):FindFirstChild("SetupGUI-WSRT"):Destroy()
end
local GUI = script["SetupGUI-WSRT"]:Clone()
...
if
	game.ReplicatedStorage:FindFirstChild("LoadLibrary")
	and not game.ServerScriptService:FindFirstChild("WeatherManager")
then
	print(
		'Redon Tech Weather System: If this game doesn\'t have the weather system installed you may remove "Load Libary" from Replicated Storage.'
	)
end
...
function IsWithinObject(Parent, Mouse)
	if
		(Mouse.X >= Parent.AbsolutePosition.X and Mouse.X <= Parent.AbsolutePosition.X + Parent.AbsoluteSize.X)
		and (Mouse.Y >= Parent.AbsolutePosition.Y and Mouse.Y <= Parent.AbsolutePosition.Y + Parent.AbsoluteSize.Y)
	then
		return true
	else
		return false
...
local MonthDropDown, updateMonthSelection = RbxGui.CreateDropDownMenu(Months, function(value)
	WeatherResources.Settings.LockMonth.Value = value
end)
...
local Weathers = {
	...
	"Rain / Snow Showers",
	"Rain / Snow",
	"Hvy Rain / Snow / TStorms",
	"Strong Thunderstorms",
}

function GetWeatherID(Weather)
	for i = 1, #Weathers do
		if Weathers[i] == Weather then
			return i
		end
	end
end
...
function IsSetUp()
	if
		game.ReplicatedStorage:FindFirstChild("WeatherResources")
		and game.ServerScriptService:FindFirstChild("WeatherManager")
		and game.StarterPack:FindFirstChild("PlayerSetup")
		and (game.StarterPlayer.StarterPlayerScripts:FindFirstChild("PrecipManager") or game.StarterPlayer.StarterCharacterScripts:FindFirstChild(
			"PrecipManager"
		))
		and game.ServerStorage:FindFirstChild("TTPPluginAdmins")
		and game.StarterGui:FindFirstChild("Admin")
	then
		return true
	else
		return false
...
function Uninstall(Directory)
	for _, Child in pairs(Directory:GetChildren()) do
		if Child:FindFirstChild("TTPPluginIdentifier") then
			if Child.TTPPluginIdentifier.Value == "Weather" then
				Child:Destroy()
			end
		end
	end
...
Main.Install.MouseButton1Click:connect(function()
	...
	local OldAdmins
	if game.ServerStorage:FindFirstChild("TTPPluginAdmins") then
		OldAdmins = game.ServerStorage.TTPPluginAdmins:Clone()
	end
	local OldSettings
	if game.ReplicatedStorage:FindFirstChild("WeatherResources") then
		if game.ReplicatedStorage.WeatherResources:FindFirstChild("Settings") then
			OldSettings = game.ReplicatedStorage.WeatherResources.Settings:Clone()
		end
	end
	...
	if not OldAdmins then
		Systems.TTPPluginAdmins:Clone().Parent = game.ServerStorage
	end
	WeatherResources = game.ReplicatedStorage.WeatherResources
...
Main.Settings.MouseButton1Click:connect(function()
	...
	if IsSetUp() then
		...
		if WeatherResources.Settings.LockMonth.Value ~= "" then
			Settings.LockMonth.Check.Visible, MonthDropDown.Visible = true, true
		else
			Settings.LockMonth.Check.Visible, MonthDropDown.Visible = false, false
		end
		if WeatherResources.Settings.LockTime.Value ~= "" then
			Settings.LockTime.Check.Visible, Settings.Time.Visible = true, true
		else
			Settings.LockTime.Check.Visible, Settings.Time.Visible = false, false
		end
...
Settings.LockMonth.MouseButton1Click:connect(function()
	if not Settings.LockMonth.GrayOut.Visible then
		...
		if Settings.LockMonth.Check.Visible then
			WeatherResources.Settings.LockMonth.Value = "January"
		else
			WeatherResources.Settings.LockMonth.Value = ""
			MonthDropDown.DropDownMenuButton.Text = "January"
		end
...
Settings.LockTime.MouseButton1Click:connect(function()
	if not Settings.LockTime.GrayOut.Visible then
		...
		if Settings.LockTime.Check.Visible then
			WeatherResources.Settings.LockTime.Value = "12:00"
		end
		...
		if not Settings.LockTime.Check.Visible then
			WeatherResources.Settings.LockTime.Value = ""
		end
	end
end)
...
for _, Child in pairs(Settings:GetChildren()) do
	Child.MouseEnter:connect(function()
		...
		elseif Child.Name == "EnableDate" then
			Help =
				"Show the current date to players.  Automatically disabled when time progression is locked to a month or frozen completely."
		elseif Child.Name == "EnableForecast" then
			Help =
				"Show the daily forecast to players.  Automatically disabled when time progression is frozen completely."
		elseif Child.Name == "LockMonth" then
			Help =
				"Lock time to a certain month to garuntee a desired seasonal weather at all times.  Disables date functionality."
...
function DisplayAdmin()
	...
	for i = 1, 24 do
		...
		if Child[i] ~= nil then
			Admin.AdminButtons["Button" .. i].Text = Child[i].Name
		else
			Admin.AdminButtons["Button" .. i].Text = ""
		end
	end
end
...
for _, Button in pairs(Admin.AdminButtons:GetChildren()) do
	Button.MouseButton1Click:connect(function()
		for _, Button2 in pairs(Admin.AdminButtons:GetChildren()) do
			if string.find(Button2.Name, "Button") ~= nil then
...
Main.Close.MouseEnter:connect(function()
	TweenHighlight(Main.Close, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Main.Climate.MouseEnter:connect(function()
	TweenHighlight(Main.Climate, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Main.Settings.MouseEnter:connect(function()
	TweenHighlight(Main.Settings, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Main.Uninstall.MouseEnter:connect(function()
	TweenHighlight(Main.Uninstall, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Main.Install.MouseEnter:connect(function()
	TweenHighlight(Main.Install, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Settings.Back.MouseEnter:connect(function()
	TweenHighlight(Settings.Back, UDim2.new(0, 35, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Settings.AdminSettings.MouseEnter:connect(function()
	TweenHighlight(Settings.AdminSettings, UDim2.new(0, 40, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Climate.Back.MouseEnter:connect(function()
	TweenHighlight(Climate.Back, UDim2.new(0, 40, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
UninstallConfirm.Back.MouseEnter:connect(function()
	TweenHighlight(UninstallConfirm.Back, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
UninstallConfirm.Uninstall.MouseEnter:connect(function()
	TweenHighlight(UninstallConfirm.Uninstall, UDim2.new(0, 50, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Admin.Back.MouseEnter:connect(function()
	TweenHighlight(Admin.Back, UDim2.new(0, 35, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Admin.Add.MouseEnter:connect(function()
	TweenHighlight(Admin.Add, UDim2.new(0, 30, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
Admin.RemoveAdmin.MouseEnter:connect(function()
	TweenHighlight(Admin.RemoveAdmin, UDim2.new(0, 30, 1, 0), UDim2.new(1, 0, 1, 0), Mouse)
end)
